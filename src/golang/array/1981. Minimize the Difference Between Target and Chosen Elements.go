...
func dp_minimizeTheDifference(mat [][]int,rows int,columns int,r int,sum int,target int,res *int,memo [][5000]bool){
	if r >= rows{
		cur := abs_int(sum - target)
		if cur < *res{
			*res = cur
		}
		return
	}
	if memo[r][sum] || sum - target >= *res{
		return
	}
	memo[r][sum] = true
	for j := 0;j < columns;j++{
		dp_minimizeTheDifference(mat,rows,columns,r + 1,sum + mat[r][j],target,res,memo)
	}
}
...
func minimizeTheDifference(mat [][]int, target int) int{
	...
	var memo [][5000]bool = make([][5000]bool,rows)
	var res int = 2147483647
	dp_minimizeTheDifference(mat,rows,columns,0,0,target,&res,memo)
	return res
}
