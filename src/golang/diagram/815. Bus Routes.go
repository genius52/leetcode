...
func NumBusesToDestination(routes [][]int, source int, target int) int {
	...
	//		var l2 int = len(routes[j])
	//		for m := 0;m < l1;m++{
	//			for n := 0;n < l2;n++{
	//				if _,ok := graph[routes[i][m]];!ok{
	//					graph[routes[i][m]] = make(map[int]bool)
	//				}
	//				graph[routes[i][m]][routes[j][n]] = true
	//				if _,ok := graph[routes[j][n]];!ok{
	//					graph[routes[j][n]] = make(map[int]bool)
	//				}
	//				graph[routes[j][n]][routes[i][m]] = true
	//			}
	//		}
	//	}
	//}
	...
	for i := 0;i < l;i++{
		...
		for j := 0;j < l1;j++{
			if routes[i][j] == source{
				...
				r.route = i
				q.PushBack(r)
				//key := strconv.Itoa(i) + "," + strconv.Itoa(routes[i][j])
		...
		for i := 0;i < cur_len;i++{
			...
			for _,other_bus_stop := range routes[cur.route]{
				...
				for _,next_route := range bus_stop_route[other_bus_stop]{
					//if next_route == cur.route{
					//	continue
					//}
					//key := strconv.Itoa(next_route) + "," + strconv.Itoa(other_bus_stop)
